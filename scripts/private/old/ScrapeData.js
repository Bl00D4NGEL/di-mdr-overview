...
const agent = httpAgent.create('www.google.com', ['/phone/']);
const baseDir = __dirname + '/../app/phones/';
const phones = [];
...
agent.addListener('next', function(error, agent) {
	const htmlPage = agent.body.replace('</head>', '</head><body>').
		replace(/<script[\s\S]*?<\/script>/gi, '');
	//  console.log(htmlPage);
	const window = jsdom.jsdom(htmlPage).createWindow();
	jsdom.jQueryify(window, 'http://code.jquery.com/jquery-1.4.2.min.js', function(window, jquery) {
		const body = jquery('body');
		if (phones.length) {
			const c1 = body.find('.g-section .g-unit:nth-child(1)');
			const c2 = body.find('.g-section .g-unit:nth-child(2)');
			var phone = {};
			...
			body.find('#thumbs img').each(function() {
				const imgUrl = 'http://www.google.com' + jquery(this).attr('src');
				phone.images.push({
					small: imgUrl,
			...
			body.find('ul.phonelist li.list').each(function(a) {
				const url = jquery(this).find('.name a').attr('href');
				console.log('=======>', url);
				const phone = {};
